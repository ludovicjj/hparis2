...
#[Route('/admin/api', name: 'app_admin_api_option_picture_')]
#[IsGranted('ROLE_ADMIN')]
class OptionPictureController extends AbstractController
{
    #[Route('/option/{id}/pictures', name: 'upload', methods: ['POST'])]
    public function upload(
        Option $option,
        Request $request,
        OptionPictureService $service,
        S3Service $s3Service,
    ): JsonResponse {
        ...
        try {
            $picture = $service->upload($option, $file);
    ...
    #[Route('/option-picture/{id}', name: 'delete', methods: ['DELETE'])]
    public function delete(
        ...
        if (!$this->isCsrfTokenValid('delete_option_picture' . $picture->getId(), $token)) {
    ...
    #[Route('/option/{id}/pictures/reorder', name: 'reorder', methods: ['POST'])]
