...
```
aws s3api put-bucket-cors --bucket h-paris-dev --cors-configuration file://cors.json --endpoint-url https://s3.fr-par.scw.cloud --profile scaleway
```

Verify current CORS config on Scaleway S3 bucket
```
aws s3api get-bucket-cors --bucket h-paris-dev --endpoint-url https://s3.fr-par.scw.cloud --profile scaleway
```